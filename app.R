...
heatmap_param_choices = c("Dissolved oxygen","Temperature, water","pH","Discharge")
names(heatmap_param_choices) = c("Dissolved oxygen", "Temperature, water", "pH", "Discharge")

ui <-fluidPage(
    
    # Header
    headerPanel(title=tags$a(href='https://workspace.oregonstate.edu/data-science-for-the-public-good-get-involved-propose-a-project',
                             tags$img(src='DSPGLogo.png', height = 75, width = 75*2.85)),
                windowTitle="River Water Quality Dashboard"),
    #
    # Title
    titlePanel("",
               tags$head(tags$link(rel = "icon", type = "image/png", href = "osufavicon.png"),
                         tags$title("River Water Quality Dashboard"))
    ),
        ...
        column(7,tabsetPanel(id="plot_tabs",
                             ...
                             tabPanel("Time series",
                                      fluidRow(column(8,
                                                      ...
                                                      radioButtons("ts_plot_type","Plot type:", choices=c("Temperature", "Temperature Projections", "Fish Population Projections"), inline=T),
                                                      ...
                                                      conditionalPanel(condition = "input.ts_plot_type =='Heatmap'",  #Come back to here
                                                                       plotOutput("heatmap")
                                                      ),
                                                      conditionalPanel(condition = "input.ts_plot_type =='Habitable width'", #Come back to here
                                                                       plotOutput("hab_width")
                                                      ),
                                                      conditionalPanel(condition = "input.ts_plot_type =='Water column exceedances'", #Come back to here
                                                                       plotOutput("pct_exc")
                                                      )
                                      ))
                             ),
...
server <- function(input, output, session){
    ...
    showModal(modalDialog(title="MAP LOADING - PLEASE WAIT...", 
                          "Please wait for map to draw before proceeding.", size="l", footer=NULL))
    ...
    au_poly1 <- data.frame(polyID = integer(),
                           ASSESS_ID = factor(),
                           AU_NAME = factor(),
                           AU_DESCRIP = factor(),
                           AU_Type = factor(),
                           geometry = list())
    data(au_poly)
    river_aus = au_poly[au_poly$AU_Type=="River/Stream",]
    
    # Extract site locations
    prof_sites = unique(assessed_profs$profile_asmnts_mlid_param[,
                                                                 c("ASSESS_ID","AU_NAME","R317Descrp","BEN_CLASS","IR_MLID","IR_MLNAME","IR_Lat","IR_Long")])
    prof_sites$MonitoringLocationTypeName = "Lake/Reservoir"
    prof_sites = plyr::rename(prof_sites, 
                              c("IR_Lat"="LatitudeMeasure", "IR_Long"="LongitudeMeasure","IR_MLID"="MonitoringLocationIdentifier","IR_MLNAME"="MonitoringLocationName"))
    ...
    session$onFlushed(once = T, function() {
        output$map <- leaflet::renderLeaflet({
            buildMap(sites = prof_sites, plot_polys = TRUE, au_poly = river_aus)
        })
    })
